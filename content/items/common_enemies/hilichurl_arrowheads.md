# id: hilichurl_arrowheads
# category: items
# item_group: common_enemies
# game_version: 1.0
# entry_type: enemy_drops

# tags: Firm Arrowhead, Sharp Arrowhead, Weathered Arrowhead, обычные противники

# title_ru: Прочный наконечник стрелы / Острый наконечник стрелы / Старый наконечник стрелы
# title_en: Arrowheads
# title_zh: 牢固的箭簇 / 锐利的箭簇 / 历战的箭簇
# region: Мондштадт, Ли Юэ, Инадзума, Сумеру, Фонтейн, Натлан, Нод-Край
# icon: assets/icons/items/common_enemies/firm_arrowhead.webp

# dropped_by: hilichurl_shooter, pyro_hilichurl_shooter, electro_hilichurl_shooter, cryo_hilichurl_shooter

# material_1_id: firm_arrowhead
# material_1_ru: Прочный наконечник стрелы
# material_1_en: Firm Arrowhead
# material_1_zh: 牢固的箭簇
# material_1_icon: assets/icons/items/common_enemies/firm_arrowhead.webp

# material_2_id: sharp_arrowhead
# material_2_ru: Острый наконечник стрелы
# material_2_en: Sharp Arrowhead
# material_2_zh: 锐利的箭簇
# material_2_icon: assets/icons/items/common_enemies/sharp_arrowhead.webp

# material_3_id: weathered_arrowhead
# material_3_ru: Старый наконечник стрелы
# material_3_en: Weathered Arrowhead
# material_3_zh: 历战的箭簇
# material_3_icon: assets/icons/items/common_enemies/weathered_arrowhead.webp

---

## RU

### material: firm_arrowhead

Грубо изготовленный наконечник стрелы. Не сильно впечатляет, однако не стоит его недооценивать. Всё, что ему требуется, - это правильный момент.

### material: sharp_arrowhead

Хорошо сделанный наконечник стрелы. Достаточно острый, чтобы проникнуть в тело с легкостью камня, проникающего сквозь поверхность воды.

### material: weathered_arrowhead

Старый наконечник стрелы, покрытый кровью. Он давно потерял свою остроту, и, следовательно, его нельзя использовать в качестве оружия.
Однако этот наконечник является гордостью охотника и выполняет роль амулета или медали.

## EN

### material: firm_arrowhead

A roughly produced arrowhead.
Though unimpressive, neither it nor the bow should be underestimated, for even the bravest knight can be felled by an arrow from the rear.

### material: sharp_arrowhead

A well-made arrowhead. Sharp enough to penetrate armor with the ease of a rock through the surface of water.

### material: weathered_arrowhead

An old arrowhead coated in blood. The arrowhead has long since lost its sharpness and thus its use as a weapon.
However, it represents the pride of a hunter and acts as both an amulet and a medal.

## ZH

### material: firm_arrowhead

做工粗糙的箭簇。即使如此，也不容小觑：弩是危险的武器，即使贵为骑士，也可能被从后方来的弩矢击倒。

### material: sharp_arrowhead

做工精良的箭簇。锐利的尖端或许能像投石入水一样轻易穿透敌人的甲胄，没入猎物的体内。

### material: weathered_arrowhead

老旧的、沾染了干涸血迹的箭簇。箭头在反复的使用中已经失去了应有的锐利，根本不能作为武器使用。
但它也代表着猎手的骄傲，是它们的护身符与奖章。

## NOTES

### Общие заметки Лороведьмы

Здесь можно оставить внутренние заметки по серии материалов.
